...
const SeminarDetailsPage = () => {
  const [seminarDetails, setSeminarDetails] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchSeminarDetails = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/landing/seminar_details/${conferenceId}`);
  ...
  if (!seminarDetails.is_registered) {
    return (
      <RegistrationMessage 
      conferenceTitle={seminarDetails.title} 
    />
    );
  }
        ...
        <div className='flex flex-col items-center justify-center min-h-[500px]'>
          ...
          <div className='lg:max-w-[60%] text-center pt-5 pb-4'>
            <p className='text-[18px] lg:text-[20px] font-[400] opacity-[0.7]'>
              <span className='text-[#D5B93C]'>Theme:</span> {seminarDetails.theme}
            </p>
          </div>
